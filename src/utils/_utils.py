...
def convert_examples_to_features(item):
    example, example_index, tokenizer, args, stage, task, sub_task, max_source_length, max_target_length = item

    if args.model_type in ['t5', 'codet5'] and args.add_task_prefix:
        if sub_task != 'none':
            source_str = "<{}> <{}> {}".format(task, sub_task, example.source)
        else:
            source_str = "<{}> <None> {}".format(task, example.source)
    else:
        source_str = example.source

    source_str = source_str.replace('</s>', '<unk>')
    source_ids = tokenizer.encode(source_str, max_length=max_source_length, padding='max_length', truncation=True)
    assert source_ids.count(tokenizer.eos_token_id) == 1
    if stage == 'test':
        target_ids = []
    else:
        # if task in ['defect', 'clone']:
        #     target_str = example.label
        # else:
        target_str = example.target
        if args.add_lang_ids:
            target_str = add_lang_by_task(example.target, task, sub_task)
        if task in ['defect', 'clone']:
            if target_str == 0:
                target_str = 'false'
            elif target_str == 1:
        ...
        target_ids = tokenizer.encode(target_str, max_length=max_target_length, padding='max_length', truncation=True)
